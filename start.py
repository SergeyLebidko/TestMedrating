...
def get_data_from_api(api_url):
    ...
    :return: полученные от api данные в формате json
    """
...
def get_user_tasks(tasks_list, user_id, completed):
    ...
    for current_task in tasks_list:
        if current_task['userId'] == user_id and current_task['completed'] == completed:
...
if __name__ == '__main__':
    ...
    # Если по какой-то причине создать каталог не удалось - выводим сообщение в консоль и завершаем работу
    try:
        check_and_create_report_folder()
    except Exception as e:
        print('При создании папки с отчетами произошла ошибка: ' + str(e))
        input('Для завершения работы нажмите Enter...')
        exit(0)
    ...
    for user in users_data:
        ...
        #  - если же при записи произошел сбой, то временный файл будет просто удален и в консоль
